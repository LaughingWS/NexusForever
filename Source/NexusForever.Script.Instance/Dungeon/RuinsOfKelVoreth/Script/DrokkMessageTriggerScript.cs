...
{ 
    ...
    public class DrokkMessageTriggerScript : IGridEntityScript, IOwnedScript<IGridTriggerEntity>
    {
        ...
        public void OnEnterRange(IGridEntity entity)
        {
            ...
            ICommunicatorMessage message = globalQuestManager.GetCommunicatorMessage(CommunicatorMessage.AvraDarkos6);
            foreach (IPlayer player in mapInstance.GetPlayers())
                message?.Send(player.Session);

            ICommunicatorMessage message2 = globalQuestManager.GetCommunicatorMessage(CommunicatorMessage.ToricAntevian5);
            foreach (IPlayer player in mapInstance.GetPlayers())
                message?.Send(player.Session);
