...
namespace NexusForever.WorldServer.Game.Loot
{
    public class LootGroup
    {
        ...
        /// </summary>
        private bool WillDrop(Player player)
        {
        ...
        private bool MeetsCondition(Player player)
        {
        ...
        /// </summary>
        public Dictionary<LootItem, uint> GenerateLootDrops(Player player)
        {
            ...
            {
                foreach ((LootItem lootItem, uint count) in GenerateLootItems(player))
        ...
        /// </summary>
        private Dictionary<LootItem, uint> GenerateLootItems(Player player)
        {
            Dictionary<LootItem, uint> itemsDropped = new Dictionary<LootItem, uint>();

            foreach (LootGroup lootGroup in childLootGroups)
                foreach ((LootItem childGroupItem, uint count) in lootGroup.GenerateLootDrops(player))
